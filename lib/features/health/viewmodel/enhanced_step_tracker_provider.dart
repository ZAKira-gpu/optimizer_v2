...
/// Enhanced step tracker provider with automatic Hive synchronization
class EnhancedStepTrackerProvider extends ChangeNotifier {
  ...
  /// Save daily steps to Firestore and sync to Hive
  Future<bool> saveDailySteps(String userId, String userLevel) async {
    try {
      ...
      if (success) {
        ...
        try {
          await _hiveSyncProvider.syncStepData(
            userId: userId,
            date: DateTime.now(),
            steps: _currentSteps,
          ...
          print('✅ Step data synced to Hive');
        } catch (e) {
          print('⚠️ Failed to sync step data to Hive: $e');

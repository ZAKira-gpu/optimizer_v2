...
/// Simplified provider for meal logging functionality without ML Kit dependencies
class SimpleMealLoggingProvider extends ChangeNotifier {
  ...
  Future<void> saveNutritionGoals(String userId) async {
    try {
      await _firestore
          .collection('userProfiles')
          .doc(userId)
  ...
  Future<List<Meal>> getMealHistory(String userId, {int limit = 10}) async {
    try {
  ...
  /// Get food suggestions from meal history
  Future<List<FoodSuggestion>> getHistorySuggestions(String userId) async {
    try {
      final meals = await getMealHistory(userId, limit: 20);
      ...
      final suggestions = foodCounts.entries
          .map(
            (entry) => FoodSuggestion(
              name: entry.key,
              confidence: 0.8, // High confidence for history
              category: 'Recent',
              description: 'Previously logged ${entry.value} time(s)',
            ),
          )
          .toList();
      ...
      // Sort by frequency
      suggestions.sort(
        (a, b) => (foodCounts[b.name] ?? 0).compareTo(foodCounts[a.name] ?? 0),
      );

      return suggestions.take(8).toList();
    } catch (e) {
      print('Error getting history suggestions: $e');
      return [];
    }
  }

  /// Get detailed meal history with filtering
  Future<List<Meal>> getDetailedMealHistory(
    String userId, {
    String? mealType,
    DateTime? startDate,
    DateTime? endDate,
    ...
    try {
      ...
      print('Error getting detailed meal history: $e');
      return [];
    }
  }
  ...
  Future<Map<String, dynamic>> getMealStatistics(
    ...
    try {
      final meals = await getDetailedMealHistory(
        userId,
        startDate: startDate,
        endDate: endDate,
      ...
      return {
        ...
        'averageCaloriesPerMeal': mealCount > 0 ? totalCalories / mealCount : 0,
      };
    } catch (e) {
  ...
  Future<bool> deleteMeal(String userId, String mealId) async {
    try {
      await _firestore
          .collection('userProfiles')
          .doc(userId)
  ...
  // Test methods removed - meal logging is working correctly
